...
class AppSignals(QObject):
    asr_partial = Signal(str)
    asr_final = Signal(str)
    asr_status = Signal(str)

    model_text = Signal(str)
    model_text_chunk = Signal(str)
    clear_bubbles = Signal()

    # 参数是本轮对话 token，防止旧线程结束后错误恢复 ASR
    conversation_finished = Signal(int)
...
class MainWindow(QMainWindow):
    def __init__(self, window_size : tuple[int, int]):
        ...
        # 当前待机动画序列（从 a_frames 或 b_frames 中选择）
        ...
        self.init_ui()
        boot_log("MainWindow UI initialized")
        self.init_workers()
        boot_log("MainWindow workers initialized")
        self.setup_window()
        boot_log("MainWindow configured")
        self.init_asr()
        boot_log("ASR preload started")

    def init_asr(self):
        """初始化新的 ASR 层。"""

        self._conversation_busy = False
        self._conversation_token = 0

        self.app_signals = AppSignals()

        self.app_signals.asr_partial.connect(
            self._on_asr_partial
        )
        self.app_signals.asr_final.connect(
            self._on_asr_final
        )
        self.app_signals.asr_status.connect(
            self._on_asr_status
        )
        self.app_signals.model_text.connect(
            self.set_model_text
        )
        self.app_signals.model_text_chunk.connect(
            self.append_model_text
        )
        self.app_signals.clear_bubbles.connect(
            self.clear_texts
        )
        self.app_signals.conversation_finished.connect(
            self._on_conversation_finished
        )

        # 兼容我们之前 config.py 的两种写法。
        cloud_config = getattr(
            config,
            "XF_API_CONFIG",
            None,
        )
        ...
        if cloud_config is None:
            ...
            if not configs:
                raise RuntimeError(
                    "没有找到讯飞 ASR 配置："
                    "XF_API_CONFIG / XF_API_CONFIGS"
                )

            cloud_config = configs[0]
    ...
    def _on_asr_partial(self, text: str):
        """ASR partial 已通过 Qt Signal 回到主线程。"""
    ...
    def _on_asr_status(self, status: str):
        ...
        if status.startswith("manager:cloud_failed:"):
            self.set_overlay_text("在线语音识别连接失败")
            return
    ...
    def _on_asr_final(self, text: str):
        ...
        # 唤醒成功。
        if action == "wake":
            self._begin_busy_session()

            token = self._conversation_token

            threading.Thread(
                target=self._play_welcome,
                args=(token,),
                daemon=True,
                name="WelcomeAudio",
            ).start()
    ...
    def _begin_busy_session(self):
        ...
        self._conversation_busy = True
        self._conversation_token += 1

        try:
            self.asr_manager.stop()
        except Exception as exc:
            logger.exception(
                f"Pause ASR error: {exc}"
            )


    def _play_welcome(self, token: int):
        try:
            # 保留原来的欢迎语和欢迎音频。
            say.choose(
                True,
    ...
    def _play_no_speak(self, token: int):
        try:
            say.choose(
                True,
                state="no_speak",
    ...
    def _process_question(
        ...
        """
        RAG -> 本地 Qwen / DeepSeek fallback -> TTS
        ...
        try:
            ...
            self._tts_queue = sentence_queue
            tts_thread = threading.Thread(
                target=lambda: asyncio.run(
                    say.speak_first_remaining(
                        self,
                        sentence_queue,
                    )
                ),
            ...
            def enqueue_tts_event(event_type, text):
                nonlocal first_sentence_enqueued

                if (
                    token == self._conversation_token
                    and not self.is_interrupt
                    and not self._closing
                ):
                    if event_type == "first":
                        if first_sentence_enqueued:
                            return
                        first_sentence_enqueued = True
                    sentence_queue.put((event_type, text))

            message_start = time.perf_counter()
            logger.info("[PERF] Before message()")
            self.set_model_text_threadsafe("")
            text = say.message(
                self,
                prompt,
                on_token=self.append_model_text_threadsafe,
                on_first_sentence=lambda text: enqueue_tts_event(
                    "first",
                    text,
                ),
                on_remaining=lambda text: enqueue_tts_event(
                    "remaining",
                    text,
                ),
            )
            logger.info(
                "[PERF] message() returned: %.3fs",
    ...
    def init_ui(self):
        ...
        label.setStyleSheet("""
                            ...
                            """)
        label.setText('\n\n'.join([f'{i}: {value}' for i, value in (self.Class.items())]))
        ...
        # 初始化打断按钮
        interrupt_icon = self.load_scaled_icon(ICON_INTERUPT, button_size - icon_margin*2)
        ...
        self.user_bubble = QLabel(self.video_label)
    ...
    def init_workers(self):
        ...
        # 启动各动画帧的加载线程
        threading.Thread(target=self.video_stream.load_frames, args=(FRAME_DIR_IN, self.video_stream.in_frames), daemon=True).start()
        threading.Thread(target=self.video_stream.load_frames, args=(FRAME_DIR_A, self.video_stream.a_frames), daemon=True).start()
        threading.Thread(target=self.video_stream.load_frames, args=(FRAME_DIR_B, self.video_stream.b_frames), daemon=True).start()
        threading.Thread(target=self.video_stream.load_frames, args=(FRAME_DIR_IDLE_C, self.video_stream.idle_frames_3), daemon=True).start()
        threading.Thread(target=self.video_stream.load_frames, args=(FRAME_DIR_C, self.video_stream.c_frames), daemon=True).start()
        ...
        # threading.Thread(target=self.video_stream.load_frames, args=(FRAME_DIR_EXIT, self.video_stream.exit_frames), daemon=True).start()
    ...
    def get_frame_for_state(self, state : str) -> QImage | None:
        ...
        if state == "entrance":
            if self.video_stream.in_frames:
                frame = self.video_stream.in_frames[self.entrance_index % len(self.video_stream.in_frames)]
                self.entrance_index += 1
                return frame
        elif state == "waiting":
            if self.video_stream.idle_frames_3:
                frame = self.video_stream.idle_frames_3[self.waiting_index % len(self.video_stream.idle_frames_3)]
                self.waiting_index += 1
                return frame
            else:
                return QImage(self.window_size[0], self.window_size[1], QImage.Format_ARGB32)
        elif state == "idle":
            if self.current_idle_frames is None:
                options = []
                if self.video_stream.a_frames:
                    options.append(self.video_stream.a_frames)
                if self.video_stream.b_frames:
                    options.append(self.video_stream.b_frames)
                    options.append(self.video_stream.b_frames)
                    options.append(self.video_stream.b_frames)
                if not options:
                    return None
                self.current_idle_frames = random.choice(options)
                self.idle_index = 0
            frame = self.current_idle_frames[self.idle_index % len(self.current_idle_frames)]
            self.idle_index += 1
            if self.idle_index >= len(self.current_idle_frames):
                self.current_idle_frames = None
            return frame
        elif state == "speaking":
            if self.video_stream.c_frames:
                frame = self.video_stream.c_frames[self.speaking_index % len(self.video_stream.c_frames)]
    ...
    def update_display(self):
        ...
        if self.animation_state == "entrance":
            ...
            if frame:
                composite = QImage(frame)
                if self.entrance_index >= len(self.video_stream.in_frames):
                    self.animation_state = "idle"
                    self.current_idle_frames = None
    ...
    def send_query(self, query: str):
        ...
        logger.info("[PERF] Question received: %r", query)
        ...
        if self.animation_state == "waiting":
            logger.info(
                "The waiting state is becoming entrance"
            )
    ...
    def on_button_clicked(self):
        logger.info(
            "The sound recognition button is clicked"
        )
    ...
    def on_interrupt_clicked(self):
        ...
        # 当前确实正在回答/播放。
        if (
            self._conversation_busy
            or self.check_audio_playing()
        ):
            self.is_interrupt = True

            # 让旧 Conversation 线程失效。
            self._conversation_token += 1
            token = self._conversation_token
    ...
    def check_audio_playing(self):
        try:
            ...
            logger.exception(
                f"Check audio status error: {exc}"
            )
            return False
            
    def clear_queue(self):
        try:
            while not say.text_queue.empty():
                say.text_queue.get_nowait()

            logger.info(
                "The queue is empty successfully"
            )
    ...
    def close_all(self):
        ...
        try:
            ...
            logger.exception(
                f"Stop audio on exit error: {exc}"
            )

        if (
            self._tts_queue is not None
            and self._tts_thread is not None
        ...
        if (
            conversation_thread
            and conversation_thread.is_alive()
            and conversation_thread is not threading.current_thread()
        ):
            conversation_thread.join(timeout=3.0)
    ...
    def set_model_text_threadsafe(self, text: str):
        """
        可以从任意后台线程调用。
        真正的 QWidget 更新会通过 Qt Signal 回到主线程。
        """
        self.app_signals.model_text.emit(text)

    def append_model_text_threadsafe(self, text: str):
        """Append one streamed model-text chunk on the Qt main thread."""
        self.app_signals.model_text_chunk.emit(text)

    def clear_bubbles_threadsafe(self):
        """
        可以从任意后台线程调用。
        """
        self.app_signals.clear_bubbles.emit()
    ...
    def append_model_text(self, text: str):
        """Append streamed model text and keep the bubble scrolled to the end."""
        self.model_text += text
        self.model_bubble.setText(self.model_text)
